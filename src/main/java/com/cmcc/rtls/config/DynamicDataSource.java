...
//                    throw new ADIException("不存在的数据源："+datasource,500);
    ...
    public void createDataSourceWithCheck(DatabaseSource dataSource) throws Exception {
        log.info("正在检查数据源："+dataSource.getUrl());
        Map<Object, Object> dynamicTargetDataSources2 = this.dynamicTargetDataSources;
        if (dynamicTargetDataSources2.containsKey(dataSource.getUrl())) {
            log.info("数据源"+dataSource.getUrl()+"之前已经创建，准备测试数据源是否正常...");
            DruidDataSource druidDataSource = (DruidDataSource) dynamicTargetDataSources2.get(dataSource.getUrl());
            boolean rightFlag = true;
            Connection connection = null;
            try {
                log.info(dataSource.getUrl()+"数据源的概况->当前闲置连接数："+druidDataSource.getPoolingCount());
                long activeCount = druidDataSource.getActiveCount();
                log.info(dataSource.getUrl()+"数据源的概况->当前活动连接数："+activeCount);
                if(activeCount > 0) {
                    log.info(dataSource.getUrl()+"数据源的概况->活跃连接堆栈信息："+druidDataSource.getActiveConnectionStackTrace());
                }
                log.info("准备获取数据库连接...");
                connection = druidDataSource.getConnection();
                log.info("数据源"+dataSource.getUrl()+"正常");
            } catch (Exception e) {
                log.error(e.getMessage(),e); //把异常信息打印到日志文件
                rightFlag = false;
                log.info("缓存数据源"+dataSource.getUrl()+"已失效，准备删除...");
                if(delDatasources(dataSource.getUrl())) {
                    log.info("缓存数据源删除成功");
                } else {
                    log.info("缓存数据源删除失败");
                }
    ...
    private  void createDataSource(DatabaseSource dataSource) throws Exception {
        log.info("准备创建数据源"+dataSource.getUrl());
        String databasetype = dataSource.getDatabasetype();
        String username = dataSource.getUserName();
        String password = dataSource.getPassWord();
        String url = "jdbc:mysql://"+dataSource.getUrl()+"?useUnicode=true&characterEncoding=UTF-8&serverTimezone=GMT%2B8&zeroDateTimeBehavior=convertToNull";
        String driveClass = "com.mysql.cj.jdbc.Driver";
        if(testDatasource(dataSource.getUrl(),driveClass,url,username,password)) {
            boolean result = this.createDataSource(dataSource.getUrl(), driveClass, url, username, password, databasetype);
            if(!result) {
                log.error("数据源"+dataSource.getUrl()+"配置正确，但是创建失败");
                throw new Exception("数据源"+dataSource.getUrl()+"配置正确，但是创建失败");
            }
        } else {
            log.error("数据源配置有错误");
